...
function sixDigits() { return randomInt(100000, 1000000).toString(); }
...
export const requestRecoveryCode=createServerFn({method:'POST'}).inputValidator((input:{identifier:string})=>{const identifier=(input.identifier??'').trim();if(identifier.length<3||identifier.length>120)throw new Error('Dato inválido');return{identifier};}).handler(async({data})=>{
  const {supabaseAdmin}=await import('@/integrations/supabase/client.server'); const raw=data.identifier; let userId:string|null=null,email:string|null=null,phone:string|null=null,fullName:string|null=null;
  ...
  const code=sixDigits();await supabaseAdmin.from('recovery_requests' as never).insert({user_id:userId,identifier:raw,channel,code,full_name:fullName,destination,delivered:false} as never);
  let delivered=false;if(channel==='whatsapp')delivered=await sendWhatsapp(destination,`Buena Vibra Finance: tu código de recuperación es ${code}. Vence en 20 minutos. Si no lo solicitaste, ignora este mensaje.`);else {const site=process.env.SITE_URL;if(site){const {error}=await supabaseAdmin.auth.resetPasswordForEmail(destination,{redirectTo:`${site}/reset-password`});delivered=!error;}}
...
export const verifyRecoveryCode=createServerFn({method:'POST'}).inputValidator((input:{identifier:string;code:string;new_password:string})=>{const identifier=(input.identifier??'').trim(),code=(input.code??'').trim(),new_password=input.new_password??'';if(!identifier||!/^[0-9]{6}$/.test(code)||new_password.length<6||new_password.length>72)throw new Error('Datos de recuperación inválidos');return{identifier,code,new_password};}).handler(async({data})=>{
  const {supabaseAdmin}=await import('@/integrations/supabase/client.server');const {data:reqs}=await supabaseAdmin.from('recovery_requests' as never).select('id,user_id,code,expires_at,attempts').eq('identifier',data.identifier).is('used_at',null).order('created_at',{ascending:false}).limit(1);const req=(reqs??[])[0] as any;
  if(!req)throw new Error('Solicitud no encontrada. Pide un código nuevo.');if(req.attempts>=5)throw new Error('Demasiados intentos. Pide un código nuevo.');if(new Date(req.expires_at).getTime()<Date.now())throw new Error('El código venció. Pide uno nuevo.');
  ...
  const {error}=await supabaseAdmin.auth.admin.updateUserById(req.user_id,{password:data.new_password});if(error)throw new Error(error.message);await supabaseAdmin.from('recovery_requests' as never).update({used_at:new Date().toISOString()} as never).eq('id',req.id);
  const {data:u}=await supabaseAdmin.auth.admin.getUserById(req.user_id);const {data:prof}=await supabaseAdmin.from('profiles').select('full_name').eq('id',req.user_id).maybeSingle();return{ok:true,email:u.user?.email??'',full_name:prof?.full_name??null,placeholder:!!u.user?.email&&isPlaceholderEmail(u.user.email)};
});

export const adminListRecoveryRequests=createServerFn({method:'POST'}).middleware([requireSupabaseAuth]).handler(async({context})=>{const {data:isAdmin}=await context.supabase.rpc('has_role',{_user_id:context.userId,_role:'admin'});if(!isAdmin)throw new Error('Solo el Administrador Principal');const {supabaseAdmin}=await import('@/integrations/supabase/client.server');const {data,error}=await supabaseAdmin.from('recovery_requests' as never).select('id,identifier,channel,code,full_name,destination,delivered,used_at,expires_at,created_at').is('used_at',null).gt('expires_at',new Date().toISOString()).order('created_at',{ascending:false}).limit(30);if(error)throw new Error(error.message);return data??[];});
